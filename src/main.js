...
const showMovie = view => {
  let template = "";
  template = `<div >
          <h2 class='left-datos color-white-text' style="background-color: rgba(15, 46, 63, 0.10);box-shadow: 0px 4px 5px rgba(7, 1, 20, 0.747)">${
            view.Title
          }</h2>
          <img src=${
            view.Poster
          } align="left" style="margin-right: 10%; overflow: hidden; padding: 2%;box-shadow: 0px 4px 5px rgba(7, 1, 20, 0.747)"></>
          <h3 class='color-white-text'>Sinopsis: ${view.Plot}</h3>
          <h3 class='left-datos color-white-text'>Género:${view.Genre}</h3>
          <h3 class='color-white-text'>Año de Estreno:${view.Year}</h3>
          <h3 class='color-white-text'>duracion: ${view.Runtime}</h3>
          <h3 class='color-white-text'>Reparto:${view.Actors}</h3>
